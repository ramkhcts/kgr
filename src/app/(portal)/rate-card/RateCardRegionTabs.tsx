...
export function RateCardRegionTabs({
  ...
  const currencySymbol = Object.values(grouped)[0]?.[0]?.currency === "EUR" ? "€" : "$";
    ...
    <div>
      ...
      <p className="text-xs text-gray-400 mb-3">
        ...
        <span className="font-600 text-gray-600">
          {Object.values(grouped)[0]?.[0]?.currency ?? "USD"}
        </span>{" "}
        · All rates are per the MSA for {REGION_LABELS[activeRegion] ?? activeRegion}
      </p>
                ...
                {rows.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-right font-700 text-[#16a34a]">
                      {currencySymbol}{r.dailyRate.toFixed(2)}
                    </td>
